...
export async function getResolvedExerciseCaloriesRange(
  ...
  // Iterate the union, not just the canonical exercise splits. A day with check-in steps
  // and no logged exercise produces no split row at all, so iterating only the splits
  // would drop that day's step calories from health-summary and 30-day tool totals.
