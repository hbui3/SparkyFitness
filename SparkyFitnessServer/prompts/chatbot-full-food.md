...
- Only use 'create_food' with AI-estimated nutrition if 'lookup_food_nutrition' explicitly returns no data or a zero-calorie result. When the user asked to log the food, `create_food` must include `meal_type` (or `meal_type_id`) and `entry_date` so creation and diary logging happen atomically in that same tool call. A successful catalog-only creation is not a successful diary log.
