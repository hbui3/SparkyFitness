...
interface ProviderDetailsFixture {
  id: string;
...
function buildProviderDetails(
  overrides: Partial<ProviderDetailsFixture>
): ProviderDetailsFixture {
  return {
    id: PROVIDER_ID,
...
describe('redactProviderDetailsForNonOwner', () => {
  ...
  it.each(['speediance', 'igpsport'] as const)(
    'redacts a %s password even when the viewer owns the provider',
    (providerType) => {
      const row = externalProviderService.redactProviderDetailsForNonOwner(
        {
          ...detailRow,
          provider_type: providerType,
          app_id: 'account@example.com',
          app_key: 'provider-password',
        },
        OWNER
      );

      expect(row.app_id).toBe('account@example.com');
      expect(row.app_key).toBeUndefined();
    }
  );
...
describe('updateExternalDataProvider - mutual exclusion + invalidation', () => {
  ...
  it('keeps stored Speediance credentials for a same-type partial update', async () => {
    vi.mocked(
      externalProviderRepository.getExternalDataProviderById
    ).mockResolvedValue(
      buildProviderDetails({
        ...
        base_url: 'https://api2.speediance.com',
      })
    );
    ...
    await externalProviderService.updateExternalDataProvider(
      OWNER,
      PROVIDER_ID,
      { provider_name: 'Renamed Speediance' }
    );
  ...
  it('does not carry iGPSPORT credentials or its regional endpoint into a new provider type', async () => {
    vi.mocked(
      externalProviderRepository.getExternalDataProviderById
    ).mockResolvedValue(
      buildProviderDetails({
