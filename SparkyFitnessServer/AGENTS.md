...
- `integrations/speediance/speedianceWorkoutService.ts`, `routes/speedianceRoutes.ts`, `routes/workoutPlanTemplateRoutes.ts`, `ai/tools/speedianceTools.ts`, and `services/plannedWorkoutScheduleService.ts` - owner-only complete remote workout editing/scheduling, atomic warm-up/set/replacement/clone transformations, native preset and 1-8 week cycle plan mirroring, German coach-variant verification, missed-session carry-forward, and the canonical range-queryable training timeline with per-day planned-versus-completed set progress
