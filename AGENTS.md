...
- The owner-only Speediance workout manager is a remote-first extension of the native workout-preset/plan domain: complete remote definitions are verified and mirrored to native presets, multi-month plans materialize through existing workout plan templates, German coach variants are mandatory, and proactive coaching carries missed planned sessions forward without introducing a second plan store. The shared canonical training timeline combines active plans, exact set/warm-up counts, completed provider workouts, missed sessions, upcoming materialized entries, and per-day planned-versus-completed progress for the Training calendar, Diary widget, and coach context.
