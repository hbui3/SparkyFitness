...
- Owner-only meal planning uses `src/pages/Settings/MealPlanningSettings.tsx`, `src/components/MealPlanning/`, `src/api/Settings/mealPlanning.ts`, and `src/hooks/Settings/useMealPlanning.ts`. It is a Nutrition & Diet settings section, not the older diary-scoped recurring meal-template calendar.
- The Speediance workout editor follows the Exercises domain mirror: `src/pages/Exercises/SpeedianceWorkoutManager.tsx`, `src/api/Exercises/speedianceWorkoutManager.ts`, and `src/hooks/Exercises/useSpeedianceWorkoutManager.ts`. It is remote-first but the server mirrors saves into native Sparky presets/plans.
- The shared training chronology uses `src/pages/Exercises/TrainingCalendar.tsx`, `src/pages/Exercises/TrainingTimeline.tsx`, `src/pages/Diary/PlannedTrainingCard.tsx`, `src/api/Exercises/workoutPlanTemplates.ts`, and `src/hooks/Exercises/useWorkoutPlans.ts`. It displays active plans and canonical completed, missed, and upcoming workouts plus per-day planned-versus-completed exercise/set progress; do not reconstruct or deduplicate this data in components.
...
- Pantry, shopping list, Coop/Migros product choice, or dated coach meal-plan UI: `src/pages/Settings/MealPlanningSettings.tsx`, `src/components/MealPlanning/`, `src/hooks/Settings/useMealPlanning.ts`, and `src/api/Settings/mealPlanning.ts`. Keep physical stock, planned reservations, and remaining shopping quantities visually distinct.
...
- Missing completed/planned workouts or wrong set/warm-up progress in the Training calendar, chronology, or Diary planned-training card: `src/pages/Exercises/TrainingCalendar.tsx`, `src/pages/Exercises/TrainingTimeline.tsx`, `src/pages/Diary/PlannedTrainingCard.tsx`, `src/hooks/Exercises/useWorkoutPlans.ts`, and the server's `plannedWorkoutScheduleService.ts`; the server response is authoritative.
