...
| **Coach Training Feedback**  | `services/trainingFeedbackService.ts`, `models/coachTrainingFeedbackRepository.ts`, `ai/tools/trainingFeedbackTools.ts`; Speediance enforcement in `integrations/speediance/speedianceWorkoutService.ts`                          | Web chat uses the shared coach tool surface            | Telegram uses the shared coach tool surface                      | `schemas/database/CoachWorkoutFeedback.zod.ts` `schemas/database/CoachTrainingPreferences.zod.ts` `schemas/api/TrainingFeedback.api.zod.ts` |
| **Speediance Workout Manager** | `integrations/speediance/speedianceWorkoutService.ts`, `routes/speedianceRoutes.ts`, `services/plannedWorkoutScheduleService.ts`, `ai/tools/speedianceTools.ts` | `pages/Exercises/SpeedianceWorkoutManager.tsx`, `api/Exercises/speedianceWorkoutManager.ts`, `hooks/Exercises/useSpeedianceWorkoutManager.ts` | Shared coach/Telegram tool surface; no separate native screen | `schemas/api/Speediance.api.zod.ts` |
...
| **Water Intake**                    | `routes/waterContainerRoutes.ts` `routes/v2/waterIntakeRoutes.ts`, `services/waterContainerService.ts`, `models/waterContainerRepository.ts` | `pages/Diary/` (water) `api/Diary/waterIntakteService.ts` _(sic — real filename)_ | `hooks/useWaterIntakeMutation.ts`                                                                | —                                                                                                                                   |
