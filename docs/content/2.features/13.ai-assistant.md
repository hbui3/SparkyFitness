...
- **Planned Training Blocks**: After explicit confirmation, the coach can create or update native multi-week workout plans from existing workout presets and assign them to fixed weekdays. These plans appear under Training > Workout Plans in the normal web interface. A completed Speediance session replaces its matching planned diary placeholder, so planned and performed copies are not counted twice.
